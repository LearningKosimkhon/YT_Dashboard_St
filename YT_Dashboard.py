...
subset_df = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo]

# Calculate the median for numeric columns in the subset
median_agg = subset_df.select_dtypes(include=['float64', 'int64']).median()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    df_agg_metrics2 = df_agg[['Video publish time']]
    metric_date_6mo = df_agg_metrics2['Video publish time'].max() - pd.DateOffset(months =6)
    metric_date_12mo = df_agg_metrics2['Video publish time'].max() - pd.DateOffset(months =12)
    metric_medians6mo = df_agg_metrics[df_agg_metrics2['Video publish time'] >= metric_date_6mo].median()
    metric_medians12mo = df_agg_metrics[df_agg_metrics2['Video publish time'] >= metric_date_12mo].median()
    ...
    df_agg_numeric_lst = df_agg_diff_final.drop('Video title', axis=1).drop('Publish_date', axis=1).median().index.tolist()
